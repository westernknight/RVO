...
namespace Pathfinding.RVO {
	...
	public class RVOController : MonoBehaviour {
        ...
        [Tooltip("[0-1]if 0 mean can slow down to 0")]
		...
		void Start()
        {
            rvoAgent.printVelocity = printVelocity;
		...
		protected void UpdateAgentProperties () {
            ...
            rvoAgent.minSpeed = Mathf.Clamp(minSpeed, 0, maxSpeed);
	...
	//#endif
		...
		public void Move (Vector3 vel) {
            if (!isPause)
            {
                ...
                if (vel == Vector3.zero)
                {
                    rvoAgent.Locked = true;
                    rvoAgent.SetNewVelocity(Vector3.zero);
                    Teleport(tr.position);
                }
            }
			else
            {
                beforePauseVelocity = vel;
            }
		}
        Vector3 beforePauseVelocity;
        ...
        public bool IsPause
        {
            ...
            set
            {
                if (value!=isPause)
                {
                    if (value)
                    {
                        beforePauseVelocity = desiredVelocity;
                    ...
                    {
                        desiredVelocity = beforePauseVelocity;
                    }
                    isPause = value;
                }
